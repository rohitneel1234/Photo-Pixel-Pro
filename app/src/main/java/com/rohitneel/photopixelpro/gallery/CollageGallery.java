...
public class CollageGallery extends AppCompatActivity {

    private static final int REQUEST_CODE_WRITE_EXTERNAL_STORAGE_PERMISSION = 1;
    private ArrayList<String> collageFileList;
        ...
        if (mOutputPath != null) {
            ...
            if (file.exists()) {
                for (File filePath : Objects.requireNonNull(file.listFiles())) {
                    if (filePath.getPath().contains("COLLAGE_")) {
                        collageFileList.add(filePath.getPath());
                    }
                }
            }
        }
            ...
            public void run() {
                ...
                if (file.exists()) {
                    ...
                    handler.post(new Runnable() {
                        ...
                        @Override
                        public void run() {
                            RecyclerView recyclerView = findViewById(R.id.rcvCollage);
                            recyclerView.setLayoutManager(new GridLayoutManager(CollageGallery.this, 1));
                            ...
                            if (collageFileList != null) {
                                ImageListAdapter adapter = new ImageListAdapter(collageFileList);
                                recyclerView.setAdapter(adapter);
                                adapter.setOnItemClickListener(new ImageListAdapter.OnItemClickListener() {
                                    @Override
                                    public void onClick(int position) {
                                        ShowDialogBox(collageFileList.get(position));
                                    }
                                });
                            }
                        }
    ...
    private void checkWarningForFileNotExist() {
        if (!file.exists() || collageFileList.isEmpty()) {
            dialog = new Dialog(CollageGallery.this);
            ...
            txtWarningMessage.setText("To view the gallery first select photos from the system gallery using the Collage Maker button!");
    ...
    private void ShowDialogBox(final String mediaPath) {
        ...
        btn_Full.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Uri uri = FileProvider.getUriForFile(CollageGallery.this, BuildConfig.APPLICATION_ID + ".provider", new File(mediaPath));
